...
def Calibrate(camera, face_mesh):
    ...
    while(flag_calib is False):
        ...
        CALIBRATING_BETWEEN_BLINKS_DURATION = []
        BLINKS_COUNTER = 0
        CEF_COUNTER = 0
        start_time = time.time()
        face_detection_start_timer = time.time()
        between_blinks_start_timer = time.time_ns() / 1000000
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if CEF_COUNTER == 1: 
                    ...
                    CALIBRATING_BETWEEN_BLINKS_DURATION.append(time.time_ns() / 1000000 - between_blinks_start_timer)
                    between_blinks_start_timer = time.time_ns() / 1000000
                if ratio > 5.5:
                    CEF_COUNTER +=1
                ...
                if lost_face_detec_dur >= FACE_LOST_DETECTION:
                    ...
                    CALIBRATING_BETWEEN_BLINKS_DURATION.clear()
                    start_time = time.time()
                    BLINKS_COUNTER = 0
                    CEF_COUNTER = 0            
                    continue
            key = cv.waitKey(2)
            if key==ord('q') or key ==ord('Q'): print("trying to exit calibrating")
        ...
        if len(CALIBRATING_BLINKS_DURATION) > 0:
            ...
            flag_calib = False
        if len(CALIBRATING_BETWEEN_BLINKS_DURATION) > 0:
            average_between_blink_dur = sum([i for i in CALIBRATING_BETWEEN_BLINKS_DURATION]) / len(CALIBRATING_BETWEEN_BLINKS_DURATION)
            if average_between_blink_dur <= 0: average_between_blink_dur = 1
        else:
            flag_calib = False
    ...
    print("AVERAGE BETWEEN BLINK DURATION: ", average_between_blink_dur)

    return flag_calib, lips_width, eyebrow_height_l, eyebrow_height_r, blinks, average_blink_dur, average_between_blink_dur
